...
/// </summary>
		// 柱の高さ
ObjectNew* Bullet::obj = nullptr;
ID3D12Device* Bullet::device = nullptr;
UINT Bullet::descriptorHandleIncrementSize = 0;
...
ComPtr<ID3D12DescriptorHeap> Bullet::descHeap;
ComPtr<ID3D12Resource> Bullet::vertBuff;
ComPtr<ID3D12Resource> Bullet::indexBuff;
ComPtr<ID3D12Resource> Bullet::texbuff;
CD3DX12_CPU_DESCRIPTOR_HANDLE Bullet::cpuDescHandleSRV;
CD3DX12_GPU_DESCRIPTOR_HANDLE Bullet::gpuDescHandleSRV;
XMMATRIX Bullet::matView{};
XMMATRIX Bullet::matProjection{};
XMFLOAT3 Bullet::eye = { 0, 0, -50.0f };
XMFLOAT3 Bullet::target = { 0, 0, 0 };
XMFLOAT3 Bullet::up = { 0, 1, 0 };
D3D12_VERTEX_BUFFER_VIEW Bullet::vbView{};
D3D12_INDEX_BUFFER_VIEW Bullet::ibView{};
Bullet::Material Bullet::material;
//Bullet::VertexPosNormalUv Bullet::vertices[vertexCount];
//unsigned short Bullet::indices[planeCount * 3];
std::vector<Bullet::VertexPosNormalUv>Bullet::vertices;
std::vector<unsigned short>Bullet::indices;
bool Bullet::StaticInitialize(ID3D12Device* device, int window_width, int window_height)
{
	// nullptrチェック
	assert(device);
...
void Bullet::PreDraw(ID3D12GraphicsCommandList* cmdList)
{
	// PreDrawとPostDrawがペアで呼ばれていなければエラー
	assert(Bullet::cmdList == nullptr);
...
Bullet* Bullet::Create()
{
	...
	float scale_val = 50.5f;
	bullet->scale = { scale_val , scale_val , scale_val };

	return bullet;
}

void Bullet::SetEye(XMFLOAT3 eye)
{
	Bullet::eye = eye;
...
void Bullet::SetTarget(XMFLOAT3 target)
{
...
void Bullet::CameraMoveVector(XMFLOAT3 move)
{
...
bool Bullet::InitializeDescriptorHeap()
{
...
void Bullet::InitializeCamera(int window_width, int window_height)
{
	// ビュー行列の生成
...
bool Bullet::LoadTexture(const std::string& directoryPath, const std::string& filename)
{
	HRESULT result = S_FALSE;
...
void  Bullet::LoadMaterial(const std::string& directoryPath, const std::string& filename)
{
	std::ifstream file;
...
void Bullet::Update(XMMATRIX matview, XMMATRIX matprojection,XMFLOAT4 color)
{
	HRESULT result;
...
void Bullet::Bullet_Initialize(ObjectNew*obj)
{
	this->obj = obj;
	position = { obj->GetPosition() };

}
void Bullet::Shot_Bullet()
{
	position.z += 5.0f;
	if (position.z >= 150) {
		Bullet_Initialize(obj);
		Bullet_Flag = 0;
	}
	else {
		Bullet_Flag = 1;
